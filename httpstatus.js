...
var HttpStatus = {
	...
	'Processing': '102',	// WebDAV : Traitement en cours (évite que le client dépasse le temps d'attente limite).
	...
	'ImUsed': '226', // RFC 32293 Le serveur a accompli la requête pour la ressource, et la réponse est une représentation du résultat d'une ou plusieurs manipulations d'instances appliquées à l'instance actuelle.
	...
	'BadMapping': '421', // Misdirected Request	La requête a été envoyée à un serveur qui n'est pas capable de produire une réponse (par exemple, car une connexion a été réutilisée).
	...
	'SSLCertificateError': '495', //Une extension de l'erreur 400 Bad Request, utilisée lorsque le client a fourni un certificat invalide.
	'SSLCertificateRequired': '496', //	Une extension de l'erreur 400 Bad Request, utilisée lorsqu'un certificat client requis n'est pas fourni.
	'HTTPRequestSenttoHTTPSPort': '497', //	Une extension de l'erreur 400 Bad Request, utilisée lorsque le client envoie une requête HTTP vers le port 443 normalement destiné aux requêtes HTTPS.
	...
	'ConnectionTimedOut': '522', //	Cloudflare n'a pas pu négocier un TCP handshake avec le serveur d'origine.
	'OriginIsUnreachable': '523', //	Cloudflare n'a pas réussi à joindre le serveur d'origine. Cela peut se produire en cas d'échec de résolution de nom de serveur DNS.
	'ATimeoutOccurred': '524', //	Cloudflare a établi une connection TCP avec le serveur d'origine mais n'a pas reçu de réponse HTTP avant l'expiration du délai de connexion.
	'SSL': '525', // Handshake Failed	Cloudflare n'a pas pu négocier un SSL/TLS handshake avec le serveur d'origine.
	'InvalidSSLCertificate': '526', //Cloudflare n'a pas pu valider le certificat SSL présenté par le serveur d'origine.
	'RailgunError': '527' //	L'erreur 527 indique que la requête a dépassé le délai de connexion ou a échoué après que la connexion WAN ait été établie.
}
